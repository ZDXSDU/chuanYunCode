    ...
    if( actionCode == 'smallLabel' ) { // 打印小标签
        if( actionCode == 'smallLabel' ) {
            ...
            if( objects == null || objects == undefined || objects == "" ) {
                ...
                setTimeout(() => {
                    $.ListView.Post( "smallLabel", {
                        OBJs: OBJs.substr( 0, OBJs.length - 1 )
                    }, ( data ) => {
                        if( data.ReturnData.res.length > 0 ) {
                            let alize = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Document</title><style>table{border-collapse: collapse;}div td {border-style: dashed;}</style></head><body><div><table><tr><td >配件号：</td><td >出库价：</td></tr><tr><td>配件名称：</td><td >适用车型：</td></tr><tr><td>位置码：</td><td >高限</td><td >低限</td></tr></table></div></body></html>`;
                            for( let i = 0;i < data.ReturnData.res.length;i++ ) {
                                ...
                                // if( ( i + 1 ) % 5 === 0 && ( i + 1 ) > 4 ) { alize += `<div class="pageTo"></div>`; }
