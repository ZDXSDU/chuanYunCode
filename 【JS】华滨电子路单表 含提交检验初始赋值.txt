$.extend( $.JForm, {
    ...
    OnValidate: function( actionControl ) {
        ...
        if( $.SmartForm.ResponseContext.ActivityCode == "Activity3" ) {
            debugger;
            var details1 = this.D117502details1;// 派车 子表业务对象
            for( var i = 0;i < details1.GetRowsCount();i++ ) {
                var licensePlateNumber = details1.GetValue()[ i ].licensePlateNumber; // 车牌号
                if( isLicenseNo( licensePlateNumber ) == 1 ) {
                    $.IShowWarn( "输入的车牌号有误，请检查后重输！示例：鲁M12345" );
                    return false;
                }
            }
